...
const Question = (props) => {
    ...
    useEffect(() => {
        ...
        // Pas de sterren aan per klik

        //Maak een nieuwe array aan
        const newStars = [];

        //Vul deze loop aan totdat je het aantal hebt berijkt van rating
        for (let i = 1; i <= rating; i++) {
            //Voeg een <i> toe in de newStars array
        ...
        //Voeg newStars array toe aan setStars (de state)
        setStars(newStars)
        props.onRate(rating, props.number)

    },
    //Run dit alleen wanneer de rating geupdatet wordt
    [rating])
